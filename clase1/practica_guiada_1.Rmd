...
La **media** es un promedio no ponderado: sumamos todos los valores de la muestra y luego dividimos por la cantidad de elementos.
...
¿Cómo la calculamos en R?

```{r}
var(c(1,2,3,4))
```

El **desvío estándar** nos brinda una información similar a la varianza, pero al calcular su raíz cuadrada, la llevamos a la misma unidad que la variable original, haciéndola más comparable.
...
Esta figura muestra ocho parcelas y sus correlaciones correspondientes. Solo cuando la relación es perfectamente lineal, la correlación es -1 o 1. Si la relación es fuerte y positiva, la correlación será cercana a +1. Si es fuerte y negativa, estará cerca de -1. Si no existe una relación lineal aparente entre las variables, entonces la correlación será cercana a cero.

Podemos calcular la correlación usando la siguiente fórmula:

$$\rho_{xy} \equiv r_{xy} = \frac{\sum_{i=1}^n (x_{i} - \overline{x}_{i})(y_{i} - \overline{y}_{i})}{\sqrt{\sum_{i=1}^n (x_{i} - \overline{x}_{i})^2} \sqrt{\sum_{i=1}^n (y_{i} - \overline{y}_{i})^2}}=\frac{\sum_{i=1}^n (x_{i} - \overline{x}_{i})(y_{i} - \overline{y}_{i})}{(n - 1)s_x s_y} =\frac{cov(x,y)}{s_x s_y} $$

La correlación es una forma "normalizada" de la _covarianza_, para que esté entre -1 y 1. Es decir que si dos variables tienen una relación lineal fuerte y positiva, podemos decir que tienen elevada correlación o elevada covarianza. Noten que el nombre "covarianza" viene de que se calcula muy parecido a la varianza (la varianza es, de hecho, la covarianza de una variable consigo misma).
...
¿Alguna relación entre variables les llama la atención?
