...
#Ruta puede variar base_juguete <- readRDS("./clase3/fuentes/eut_juguete.RDS")
```
...
```{r}
lm_vif <- lm(ingreso_individual ~ . , #Con . indicamos "todas las demás"
               data = base_juguete)
...
No sólo es significativo el coeficiente de la interacción, sino que además mejoró el $R^{2}$ ajustado.
...
Le dimos la base de datos, pero no estimamos nada. Solo necesita la base de datos para saber qué variables hay y de qué tipo son. Podemos ver cómo quedarán los datos previo al ajuste del modelo:
