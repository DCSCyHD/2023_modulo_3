...
<br>
**Aclaración**: Por un error de tidymodels, si queremos correr KNN con variables explicativas categóricas debemos cargar adicionalmente el paquete "kknn". Acá elegimos no incluirlas para no sumar otros paquetes más
<br>
```{r}
...
# Comparando modelos    
Una de las potencialidades de **tidymodels** es que, al ordenar las estimaciones de los modelos en formato tidy, nos permite con mucha facilidad comparar métricas de cada uno de ellos. Adicionemos a los dos modelos vistos recién una regresión logística (como la de la clase pasada).      
Crando una lista de todos los modelos a evaluar, puedo luego con la función `imap_dfr` pasar cada uno de estos modelos como argumento de la función **augment()** y así agregar las predicciones de cada modelo al set de testing. 

```{r}
logistica_fiteada <-  logistic_reg() %>% 
  fit(formula = realiza_trabajo_domestico  ~ horas_trabajo_mercado +ingreso_familiar,
  data = base_train
  ) 
...
#La función imap_dfr una lista de modelos#

base_test_preds <- imap_dfr(modelos,
                  augment, 
                  new_data = base_test, .id = "model")

#Veamos
base_test_preds %>%
  select(model, realiza_trabajo_domestico, .pred_class, .pred_No, .pred_Si)
```
...
Si agrupamos por modelo, podemos consultar para cada uno de llos las metricas en cuestión.
```{r}
base_test_preds %>%
  group_by(model) %>%
