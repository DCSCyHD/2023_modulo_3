---
title: "Práctica independiente 5 - Módulo 3"
subtitle: "LASSO"
author: 
...
#Especifico modelo Lasso y fiteo
lasso_model <- logistic_reg(penalty = 0.01,
                      mixture = 1) %>% 
  set_mode("classification") %>% 
  set_engine("glmnet") 

lasso_rec <- recipe(realiza_trabajo_domestico ~ .,data = train_data) %>%
...
names(matrices_conf$conf_mat) <- c("LASSO","logistica")
...
- Suponga que el modelo en cuestión se utiliza para definir una política en la cual el Estado le otorga una transferencia de dinero a las personas que se estima que realizan trabajo doméstico no remunerado. Su jefe le aclara expresamente que **es preferible equivocarse y que la transferencia se otorgue a personas que NO realizan trabajo doméstico respecto a equivocarse y que no se otorgue a quien SI realiza**.   

  - ¿Con cuál de los modelos se quedaría?, ¿qué metrica resulta más importante para definir ello?
  
  - Una vez tomada la decisión sobre cual modelo elegir, le muestra la matriz de confusión a su jefe y este le plantea lo siguiente: **"El nivel de personas que realiza TDNR, pero no recibirían la transferencia dado este modelo  sigue siendo muy alto"**. ¿Qué puede hacer con este mismo modelo para satisfacer la inquietud de su jefe? **Pista**: Miren la variable `.pred_class`, del data frame. **base_test_preds** Luego miren `.pred_Si` y `.pred_No`  ¿Cómo se está definiendo `.pred_class`?
  
```{r echo=TRUE}
#
```
  - Creen una nueva variable llamada "transferir" y tome los valores "Si" o "No". Traten de establecer una **regla de decisión** que satisfaga un poco más la inquietud de su jefe. Luego recalculen la matriz de confusión y algunas de las métricas asociadas

```{r include=FALSE}
base_test_preds %>% 
  filter(model == "lasso") %>% 
  mutate(transferir = case_when(.pred_Si >= 0.45 ~  "Si",
                               .pred_Si < 0.45 ~  "No")) %>% 
