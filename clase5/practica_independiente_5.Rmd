...
base_spliteada  <- initial_split(base_muestra)
train_data <- training(base_spliteada)
test_data <- testing(base_spliteada) 
...
base_test_preds %>% 
  filter(model == "KNN") %>% 
  mutate(transfeir = case_when(.pred_Si >= 0.25 ~  "Si",
                               .pred_Si < 0.25 ~  "No")) %>% 
  conf_mat(truth = realiza_trabajo_domestico,transfeir)
```
