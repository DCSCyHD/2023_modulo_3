...
author: 
  - Diplomatura en Ciencias Sociales Computacionales y Humanidades Digitales (IDAES-UNSAM). Agosto/Septiembre 2023
  - Carolina Pradier, Guido Weksler y Karina Bartolomé
output: 
  html_document:
    toc: TRUE
    toc_float:
...
Vamos a usar una base de **juguete** (es decir, con datos inventados), cuyas variables tiene nombres similares a los de la base de la Encuesta de Uso del Tiempo. Inventamos datos nuevos para que las relaciones sean claramente lineales (esto no suele pasar en ciencias sociales). Se busca entender la relación entre distintas características de uso del tiempo con el salario.
...
Con el paquete equatiomatic es posible extraer la ecuación del modelo ajustado en formato Latex (muy útil para realizar investigación académica).

```{r}
equatiomatic::extract_eq(lm_fit$fit)
```

Con el parámetro use_coefs=TRUE se reemplazan las letras griegas por los coeficientes estimados en el modelo:

```{r}
equatiomatic::extract_eq(lm_fit$fit, use_coefs = TRUE)
```
