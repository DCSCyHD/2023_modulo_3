...
*Aclaración:* La variable objetivo **realiza_trabajo_domestico** debo convertirla en factor para correr una regresión logistica (es decir, que sea una variable con categorías).
...
```{r}
matriz_confusion <- conf_mat(base_con_pred,
...
```{r}
accu <- (matriz_confusion$table[2,2]+matriz_confusion$table[1,1])/sum(matriz_confusion$table)
error_rate <- 1-accu
Sensitivity <- matriz_confusion$table[2,2]/sum(matriz_confusion$table[,2])
Specificity <- matriz_confusion$table[1,1]/sum(matriz_confusion$table[,1])
precision <- matriz_confusion$table[2,2]/sum(matriz_confusion$table[2,])
```
 - El modelo predice correctamente un `r scales::percent(accu,accuracy = 0.01) ` de los casos.     
 - De los casos positivos (que efectivamente realizan trabajo doméstico), el modelo predice bien un `r scales::percent(Sensitivity,accuracy = 0.01) `.     
 - De los casos negativos (que no realizan trabajo doméstico), el modelo predice bien un `r scales::percent(Specificity,accuracy = 0.01) `.    
 - De los casos que son clasificados como positivos, sólo un `r scales::percent(precision,accuracy = 0.01)` . lo son.
...
Hagamos un modelo nuevo con un split de la base original en test y train.    
...
```{r}
base_test_con_pred <- augment(x = modelo_sobre_train, base_test)
base_test_con_pred
```
### Matriz de confusión
Sobre la base de los valores reales y los valores predichos podríamos tomar la matriz de confusión como un tabulado bivariado entre ambas columas
