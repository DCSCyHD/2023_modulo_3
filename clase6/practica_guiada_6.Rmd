...
#Ruta puede variar "clase6/fuentes/eut_juguete.RDS"
set.seed(42)
...
¿Qué contiene **lm_valfit**? Para cada fold (que se corresponde con una iteración distinta) contiene las métricas y las predicciones. 
...
El cross-validation se usa para comparar las predicciones "fuera de la muestra", y con eso determinar cuál es el mejor modelo. Recien lo aplicamos a distintas recetas de un mismo tipo de modelo (regresión lineal), pero también podríamos usarlo para:
...
Para los primeros dos casos, alcanza con ajustar el workflowset. El último es lo que se conoce como *tuneo* ("espanglish" de tune: afinar). Consiste en evaluar cual es el mejor valor posible para un hiperparámetro en función de la capacidad de predecir fuera de la muestra.
...
Definimos también 2 modelos: un LASSO cuyo $\lambda$ vamos a tunear, y un logit.
...
```{r}
grilla <- tibble(penalty=seq(from = 0.01, by = 0.01, length.out = 4))
...
Se muestra cada métrica para todos los modelos, y por default los resultados se ordenan según la primera.

```{r}
wflowset_fit %>% autoplot()
```

¿Como dan las métricas para cada valor del hiperparámetro de un workflow específico?

```{r}
# Se extrae el workflow
wflowset_fit %>% 
  extract_workflow_set_result(id = 'extendida_lasso') %>% 
  collect_metrics()

wflowset_fit %>% 
  extract_workflow_set_result(id = 'basica_lasso') %>% 
  collect_metrics()

wflowset_fit %>% 
  extract_workflow_set_result(id = 'extendida_logit') %>% 
  collect_metrics()
